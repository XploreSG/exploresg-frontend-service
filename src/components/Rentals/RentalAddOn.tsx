...
const RentalAddOn: React.FC = () => {
  ...
  const bookingDetails: BookingDetails = {
    ...
    petFriendly: true,
    nights: 5,
    basePrice: 250, // Base price per night
  };

  const [selectedCDW, setSelectedCDW] = useState<string>("basic");
  const [selectedAddOns, setSelectedAddOns] = useState<Record<string, boolean>>(
    {
      "malaysia-drive": true, // Pre-select included items
    },
  );
    ...
    <div className="mx-auto max-w-4xl rounded-xl bg-white p-6 shadow-lg">
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 grid grid-cols-1 gap-2 text-sm sm:grid-cols-3">
            ...
            <div>
              ...
              <div className="font-medium">{bookingDetails.nights} nights</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-lg font-semibold">Collision Damage Waiver</h3>
        ...
        <div className="mt-4 rounded-lg border border-yellow-200 bg-yellow-50 p-3">
          <p className="text-sm text-yellow-800">
            <strong>Insurance Excess:</strong> Amount you pay in case of damage
            - Singapore: S$2,000 | Malaysia: S$4,000
          </p>
        </div>
      </div>
      ...
      <div className="mb-6 rounded-lg bg-gray-50 p-4">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span>
              S$ {(bookingDetails.basePrice * bookingDetails.nights).toFixed(2)}
            </span>
          </div>
          ...
          {selectedCDW !== "basic" && (
            <div className="flex justify-between">
              <span>
                Insurance upgrade (
                {CDW_OPTIONS.find((opt) => opt.key === selectedCDW)?.label})
              </span>
              <span>
                S${" "}
                {(
                  (CDW_OPTIONS.find((opt) => opt.key === selectedCDW)?.price ||
                    0) * bookingDetails.nights
                ).toFixed(2)}
              </span>
            </div>
          )}
      ...
      <div className="mt-6 rounded-lg bg-gray-50 p-4">
        ...
        <ul className="space-y-1 text-sm text-gray-700">
          ...
          <li className="flex items-start">
            <span className="mr-2 text-blue-500">•</span>
            <span>
              Entry to West Malaysia is allowed with daily surcharge of S$25
            </span>
          </li>
          <li className="flex items-start">
            <span className="mr-2 text-blue-500">•</span>
            <span>
              Free cancellation & 100% refund if pickup is more than 72 hours
              away
            </span>
          </li>
          <li className="flex items-start">
            <span className="mr-2 text-blue-500">•</span>
            <span>Valid driving license required for at least 1 year</span>
          </li>
        </ul>
